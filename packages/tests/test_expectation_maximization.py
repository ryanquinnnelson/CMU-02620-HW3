...
def test_e_step():
    ...
    expected_k0 = np.round(all_prob[0].reshape(-1, 1) / p_X, decimals=5)
    expected_k1 = np.round(all_prob[1].reshape(-1, 1) / p_X, decimals=5)
    expected_k2 = np.round(all_prob[2].reshape(-1, 1) / p_X, decimals=5)
